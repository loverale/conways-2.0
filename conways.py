        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if self.grid[row][col] == 1:
                    ...
                    # SURVIVORSHIP RULES - just comment out the line when not using
                    new_grid[row][col] = 1 if alive_neighbors in [2, 3] else 0 # default
                    #new_grid[row][col] = 1 if alive_neighbors >= 2 else 0 # removes death by overpopulation
                    new_grid[row][col] = 1 if alive_neighbors in [1, 4] else 0 # death on values of 2, or >= 5

                else:

                    # POPULATION RULES - just comment out the line when not using
                    new_grid[row][col] = 1 if alive_neighbors == 3 else 0 # default (i.e., needs exactly 3 to populate)
